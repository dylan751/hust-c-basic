...
NodeType *createNode(KeyType *NewData){
  NodeType *N = NULL;
...
void insertNode(KeyType x[], TreeType *Root){
  ...
  else if(so_sanh(x, (*Root)->key) < 0) insertNode(x, &((*Root)->left));
  else if(so_sanh(x, (*Root)->key) > 0) insertNode(x, &((*Root)->right));
}
