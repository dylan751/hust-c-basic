...
treeType search(char* x, treeType root){
  ...
  else if(compare(root->element.model,x) == 0){//Neu tim thay
    return root;
  }
  else if(compare(root->element.model,x) < 0){//Neu < khoa can tim -> tiep tuc tim ben phai
...
void insertNode(elementType x, treeType *root){    //Neu truyen vao con tro -> thay doi ca tree. Neu chi truyen vao root bthg -> phai return de lay gia tri
  ...
  else if(compare(x.model, (*root)->element.model) < 0)//Neu khoa them vao < khoa nut goc 
    insertNode(x, &(*root)->left);
  
  else if(compare(x.model, (*root)->element.model) > 0)
...
void inOrderPrint(treeType root){
  if(root != NULL){
    ...
    printf("%30s%10s%3.2lf%3.2lf\n", root->element.model, root->element.memory,root->element.size, root->element.price);
...
void preOrderPrint(treeType root){
  if(root != NULL){
    printf("%30s%10s%3.2lf%3.2lf", root->element.model, root->element.memory,root->element.size, root->element.price);
...
void deleteNode(char* x, treeType *root){
  if(*root != NULL){
    if(compare(x, (*root)->element.model) < 0){
      deleteNode(x, &(*root)->left);
    }
    else if(compare(x, (*root)->element.model) > 0){
...
//23. Duyet cay theo chieu rong (xem slide Cao Tuan Dung)
