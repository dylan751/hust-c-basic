...
treeType search(keyType x, treeType root){
  ...
  else if(compare(root->element.english, x) == 0){//Neu tim thay
    return root;
  }
  else if(compare(root->element.english, x) < 0){//Neu < khoa can tim -> tiep tuc tim ben phai
...
void insertNode(elementType x, treeType *root){    //Neu truyen vao con tro -> thay doi ca tree. Neu chi truyen vao root bthg -> phai return de lay gia tri
  ...
  else if(compare(x.english, (*root)->element.english) < 0)//Neu khoa them vao < khoa nut goc 
    insertNode(x, &(*root)->left);
  
  else if(compare(x.english, (*root)->element.english) > 0)
...
void deleteNode(keyType x, treeType *root){
  if(*root != NULL){
    if(compare(x, (*root)->element.english) < 0){
      deleteNode(x, &(*root)->left);
    }
    else if(compare(x, (*root)->element.english) > 0){
