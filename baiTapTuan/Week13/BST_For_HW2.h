...
typedef int keyType; //Kieu dung de so sanh
...
void deleteNode(char* x, treeType *root){
  if(*root != NULL){
    if(compare(x, (*root)->element.name) < 0){
      deleteNode(x, &(*root)->left);
    }
    else if(compare(x, (*root)->element.name) > 0){
...
int lowerThanKey(keyType x, treeType root){
  if(root == NULL){
    return 0;
  }

  if(x > root->element.grade){
     printf("%-25s%.2f\n", root->element.name, root->element.grade);
  }
...
int higherThanKey(keyType x, treeType root){
  if(root == NULL){
    return 0;
  }
