...
treeType search(char* x, treeType root){
  ...
  else if(compare(root->element.name, x) == 0){//Neu tim thay
    return root;
  }
  else if(compare(root->element.name, x) < 0){//Neu < khoa can tim -> tiep tuc tim ben phai
...
void insertNode(elementType x, treeType *root){    //Neu truyen vao con tro -> thay doi ca tree. Neu chi truyen vao root bthg -> phai return de lay gia tri
  ...
  else if(compare(x.name, (*root)->element.name) < 0)//Neu khoa them vao < khoa nut goc 
    insertNode(x, &(*root)->left);
  
  else if(compare(x.name, (*root)->element.name) > 0)
...
void deleteNode(elementType x, treeType *root){
  if(*root != NULL){
    if(compare(x.name, (*root)->element.name) < 0){
...
//23. Tim cac phan tu co gia tri nho hon x
int lowerThanKey(int x, treeType root){
  if(root == NULL){
    return 0;
  }

  if(x > root->element.score){
     printf("%-10s%-5d\n", root->element.name, root->element.score);
  }
...
int higherThanKey(int x, treeType root){
  if(root == NULL){
    return 0;
  }
...
void deleteLowerThanKey(int x, treeType *root){
  if(root == NULL){
    return;
  }

  deleteLowerThanKey(x, &((*root)->right));
  
  deleteLowerThanKey(x, &((*root)->right));
  if(x > (*root)->element.score){
    deleteNode((*root)->element, &(*root));
  }
}
